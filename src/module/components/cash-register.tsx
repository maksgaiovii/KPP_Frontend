...
const ButtonWithText: React.FC<ButtonWithTextProps> = ({ position, text, color, onClick }) => {
  return (
    <mesh position={position} rotation={[-Math.PI / 20, 0, 0]} onClick={(e) => {
        e.stopPropagation();
        onClick();
    }}>
...
export const CashRegister: React.FC<CashRegisterProps> = ({ position }) => {
  ...
  */
  ...
  return (
    <group position={position}>
      ...
      <ButtonWithText position={[0, 0.1, 0.12]} text="Margherita" color="#ffdd57" onClick={() => handlePizzaSelection('Margherita')} />
      <ButtonWithText position={[0, 0.2, 0.11]} text="Pepperoni" color="#ff5733" onClick={() => handlePizzaSelection('Pepperoni')} />
      <ButtonWithText position={[0, 0.3, 0.095]} text="Hawaiian" color="#ff3e00" onClick={() => handlePizzaSelection('Hawaiian')} />
      <ButtonWithText position={[0, 0.4, 0.08]} text="Veggie" color="#9bcd56" onClick={() => handlePizzaSelection('Veggie')} />
      <ButtonWithText position={[0, 0.5, 0.065]} text="Cheese" color="#f7c8b8" onClick={() => handlePizzaSelection('Cheese')} />
